...
class View(ft.UserControl):
    ...
    def load_interface(self):
        ...
        self._txtInNumC = ft.TextField(label="Num compagnie min", width=150)
        self._btnAnalizza = ft.ElevatedButton(text="Analizza Aeroporti",
                                              on_click=self._controller.handleAnalizza,
                                              width=150)
        row1 = ft.Row([
            ft.Container(self._txtInNumC, width=400),
            ft.Container(self._btnAnalizza, width=200)],
            alignment=ft.MainAxisAlignment.CENTER)

        self._page.controls.append(row1)

        self._ddAeroportoP = ft.Dropdown(label="Partenza", width=400, disabled=True)
        ...
        self._ddAeroportoA = ft.Dropdown(label="Arrivo", width=400, disabled=True)
        ...
        self._txtInNumTratte = ft.TextField(label="Num Tratte Max", width=200, disabled=True)
        self._btnCercaItinerario = ft.ElevatedButton(text="Cerca Itinerario",
                                                     on_click=self._controller.handleCercaItinerario,
                                                     width=200,
                                                     disabled=True)
        row4 = ft.Row([
            ft.Container(self._txtInNumTratte, width=400),
            ft.Container(self._btnCercaItinerario, width=200)],
            alignment=ft.MainAxisAlignment.CENTER)
